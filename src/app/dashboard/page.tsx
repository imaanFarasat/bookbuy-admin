...
import ContentAssistant from '@/components/ui/content-assistant'
...
export default function Dashboard() {
  ...
  const generateAllComponents = async () => {
    ...
    try {
      // Step 1: Generate content for all keywords as one unique article
      setMessage('🤖 Step 1/3: Generating content for all keywords as one unique article...')
      
      // Generate fresh AI content for all selected keywords together
      setMessage('🤖 AI is generating unique content for all keywords as one article...')
      
      let mainContentWithAI = []
      
      try {
        console.log(`Generating content for all keywords: ${selectedKeywords.map(k => k.keyword).join(', ')}`)
        ...
        const result = await response.json()
        console.log(`API response for all keywords:`, result)
        
        // Parse the generated content to extract individual sections
        const generatedContent = result.content || ''
        
        // Extract content for each keyword from the generated HTML
        for (const keyword of selectedKeywords) {
          const keywordTitle = keyword.keyword.split(' ').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ')
          
          // Look for the specific H2 section in the generated content
          const h2Pattern = new RegExp(`<h2[^>]*>${keywordTitle}[^<]*</h2>.*?<p class="p-body-content">([^<]+)</p>`, 's')
          const match = generatedContent.match(h2Pattern)
          ...
          if (match) {
            ...
            // Try alternative parsing - look for any content after the heading
            const alternativePattern = new RegExp(`<h2[^>]*>${keywordTitle}[^<]*</h2>.*?<p[^>]*>([^<]+)</p>`, 's')
            const altMatch = generatedContent.match(alternativePattern)
            ...
            if (altMatch) {
              ...
              // Try to find content by keyword in the text
              const keywordPattern = new RegExp(`${keyword.keyword}[^<]*`, 'i')
              const keywordMatch = generatedContent.match(keywordPattern)
              
              if (keywordMatch) {
                // Extract surrounding content
                const startIndex = Math.max(0, generatedContent.indexOf(keywordMatch[0]) - 100)
                const endIndex = Math.min(generatedContent.length, generatedContent.indexOf(keywordMatch[0]) + keywordMatch[0].length + 200)
                const extractedContent = generatedContent.substring(startIndex, endIndex)
                
                mainContentWithAI.push({
                  keyword: keyword.keyword,
                  content: extractedContent.replace(/<[^>]*>/g, '').trim(),
                  headingType: keyword.headingType
                })
              } else {
                // Final fallback - create informative content about the keyword
                mainContentWithAI.push({
                  keyword: keyword.keyword,
                  content: `Comprehensive information about ${keyword.keyword}. This section provides detailed insights, tips, and expert advice to help you understand and make the most of ${keyword.keyword}. Whether you're a beginner or experienced user, you'll find valuable information here to enhance your knowledge and skills.`,
                  headingType: keyword.headingType
                })
              }
            }
        ...
        console.error(`Error generating content for all keywords:`, error)
        // Fallback content for each keyword
        mainContentWithAI = selectedKeywords.map(keyword => ({
          keyword: keyword.keyword,
        ...
        <div className="mt-8">
          <ContentAssistant 
            onContentGenerated={(content) => {
              console.log('Content generated by assistant:', content)
              // You can integrate this with your existing content system
            }}
            onFAQGenerated={(faqContent) => {
              console.log('FAQ generated by assistant:', faqContent)
              // You can integrate this with your existing FAQ system
            }}
            onSchemaGenerated={(schema) => {
              console.log('Schema generated by assistant:', schema)
              // You can integrate this with your existing schema system
            }}
          />
        </div>
